...
export const AddDiagnose = ({dataList, setDataList}) => {
  ...
  const handleSearch = (text: string) => {
    getDiagnoseMasteApi(text);
    setSearchValue(text);
  };

  const getDiagnoseMasteApi = async (param = '') => {
    setIsLoading(true);
    const res = await clinicalNotesMaster('diagnose', param);
    if (res) {
      setIsLoading(false);
